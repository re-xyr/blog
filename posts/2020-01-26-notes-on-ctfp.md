...
## 物件（Object）$A, B, ...$ 及其类 ${\rm ob}(\mathcal C)$

物件...就是物件。它是一个抽象的概念。

物件一般通过斜体大写字母表示。

记号 ${\rm ob}(\mathcal C)$ 表示了范畴 $\mathcal C$ 的物件的类。

## 态射（Morphism）$f: A \to B, ...$ 及其类 ${\rm hom}(\mathcal C)$
...
记号 $\rm hom(\mathcal C)$ 表示了范畴 $\mathcal C$ 的态射的类。

## 两个物件之间的态射类 $\mathcal C(A, B), ...$

记号 $\mathcal C(A, B)$（有时表示为 ${\rm hom}_{\mathcal C}(A, B)$）是范畴 $\mathcal C$ 内从 $A$ 至 $B$ 的态射类。
...
同构（Isomorphism）是一组态射 $\langle f: X \to Y, g: Y \to X \rangle$，满足 $g \circ f = {\rm id}_X$ 且 $f \circ g = {\rm id}_Y$。
...
代入 ${\bf Set}$ 去理解，任意两个集合 $A$、$B$ 的积是它们的 Cartesian 积 $A \times B$（想一想，为什么），它包含了所有有序对 $\langle A, B \rangle$ 当 $a \in A$、$b \in B$。
...
- $\langle {\rm ob}({\bf Set}), \times \rangle$ 构成一个幺半群，其中幺元为仅有一个元素的集合 $\mathbb{S}$。它还有一个零元，即空集 $\varnothing$（想一想，为什么）。
- $\langle {\rm ob}({\bf Set}), \oplus \rangle$ 构成一个幺半群，其中幺元为空集 $\varnothing$。
...
双函子是函子的一个推广。它是这样一个态射 $G: \langle \mathcal A, \mathcal B \rangle \to \mathcal C$，也就是说它将两个范畴的 Cartesian 积映射到一个范畴。
...
### 协变（Covariant）函子与反变（Contravariant）函子

普通的函子有时候又叫协变函子，这时是为了与另一种不同的反变函子相区分。

对于范畴 $\mathcal C$、$\mathcal D$，一个反变函子是这样一个态射 $G: \mathcal C^{op} \to \mathcal D$，即它从一个范畴的对偶范畴射向另一个范畴。

### Haskell 下的讨论

如果你学过 Haskell 中的 Functor，这些概念应该可以很容易地找到 Haskell 中的对应。在 Haskell 中若存在 Functor `f`，则 `f a` 是对物件 `a` 的映射，而 `fmap n` 是对态射 `n` 的映射。`Maybe`、`Either a`、`[a]`、`(a, )` 等一大票你熟知的类型都是函子。而且，它们都是 ${\bf Hask}$ 下的自函子（为什么）。
